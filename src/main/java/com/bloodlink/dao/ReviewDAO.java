...
public final class ReviewDAO {
    ...
    /**
     * Whether this reviewer already rated <em>this donor</em> on this request.
     * <p>
     * One request can be filled by several donors, each with their own
     * handshake, so "have you reviewed this request" is the wrong question --
     * it would let a requester rate the first donor and then silently refuse
     * to let them rate the second.
     */
    public boolean hasReviewed(long requestId, long reviewerId, long reviewedId) throws SQLException {
        String sql = "SELECT 1 FROM reviews WHERE request_id=? AND reviewer_id=? AND reviewed_id=?";
        try (Connection connection = DBConnection.getConnection(); PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setLong(1, requestId);
            statement.setLong(2, reviewerId);
