...
public final class HospitalDAO {
    ...
    /** How much of the country the directory currently covers. */
    public record Coverage(int hospitals, int districts) { }

    /**
     * Counts what is actually in the directory right now.
     * <p>
     * One query rather than two: both figures come off the same scan, and the
     * sign-in screen shows them together, so splitting it would mean two round
     * trips to display one fact.
     */
    public Coverage coverage() throws SQLException {
        String sql = "SELECT COUNT(*) AS facilities, COUNT(DISTINCT district) AS districts FROM hospitals WHERE active=TRUE";
        try (Connection connection = DBConnection.getConnection();
             PreparedStatement statement = connection.prepareStatement(sql);
             ResultSet rs = statement.executeQuery()) {
            if (rs.next()) return new Coverage(rs.getInt("facilities"), rs.getInt("districts"));
        }
