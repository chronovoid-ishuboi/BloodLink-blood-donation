...
public final class ProfileService {
    ...
    /**
     * Takes the account out of matching without destroying anything. Reversible
     * by signing in again, which is the whole difference between this and
     * {@link #deleteAccount}.
     */
    public ServiceResult<Void> deactivateAccount(long userId) {
        try {
            authorizationService.requireSelfOrAdmin(userId);
            userDAO.setActive(userId, false, userId);
            return ServiceResult.success("Your account is deactivated. Sign in again to reactivate it.", null);
        } catch (SQLException e) {
     ...
     * <p>
     * A donor's verified donation history is referenced by blood_requests with
     * ON DELETE RESTRICT, so a donor who has actually donated cannot simply be
     * deleted without destroying the other party's record of a real
     * transfusion. That is refused explicitly rather than failing with a
     * foreign-key error: the honest answer is that the account can be
     * deactivated and anonymised, not erased, once it is part of someone
     * else's medical history.
     */
    public ServiceResult<Void> deleteAccount(long userId) {
        try {
            authorizationService.requireSelfOrAdmin(userId);
            if (userDAO.hasDonationHistory(userId)) {
                return ServiceResult.failure("""
                        This account has verified donations attached to other people's requests, \
                        which are part of their medical record and cannot be deleted.

                        You can deactivate the account instead, and turn on anonymous mode so your \
                        name no longer appears anywhere.""");
            }
            userDAO.deleteAccount(userId);
            return ServiceResult.success("Your account and its data have been deleted.", null);
        } catch (SQLException e) {
            return ServiceResult.failure(e.getMessage());
        }
    }

    /** Everything held about this user, as JSON, for the data export. */
    public String exportAccountData(long userId) throws SQLException {
        return userDAO.exportAccountData(userId);
    }
