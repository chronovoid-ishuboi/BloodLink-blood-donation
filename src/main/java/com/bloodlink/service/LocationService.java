...
public final class LocationService {
    ...
    /**
     * Where a specific user actually is, with the source attached, resolved in
     * descending order of how much it can be trusted: a position they pinned
     * themselves, then an auto-detected network estimate, then their chosen
     * reference hospital, then their district's reference point.
     * <p>
     * Everything below MAP_PIN is an estimate shared by many people, which is
     * exactly why {@link UserLocation#source()} travels with the coordinates
     * rather than being dropped here -- the map labels pins with it instead of
     * drawing a district centroid as though it were somebody's doorstep.
     */
    public Optional<UserLocation> resolvedLocation(long userId, String district, Long referenceHospitalId) {
        try {
            Optional<UserLocation> stored = userDAO.findLocation(userId);
            if (stored.isPresent()) return stored;
        } catch (SQLException e) {
            // Fall through to the estimates below rather than failing the map.
        }
        if (referenceHospitalId != null) {
            Optional<double[]> chosen = referenceHospitalPoint(referenceHospitalId);
            if (chosen.isPresent()) {
                return chosen.map(point -> new UserLocation(point[0], point[1], UserLocation.Source.DISTRICT, null));
            }
        }
        return districtReferencePoint(district)
                .map(point -> new UserLocation(point[0], point[1], UserLocation.Source.DISTRICT, null));
    }

    /** Stores a position the user established themselves, or one auto-detected for them. */
    public void saveLocation(long userId, double latitude, double longitude, UserLocation.Source source) throws SQLException {
        userDAO.updateLocation(userId, latitude, longitude, source.name());
    }

    /** How far this user will travel, in km. Falls back to the schema default rather than failing the screen. */
    public int travelRadiusKm(long userId) {
        try {
            return userDAO.findTravelRadiusKm(userId);
        } catch (SQLException e) {
            return 15;
        }
    }

    public void saveTravelRadius(long userId, int radiusKm) throws SQLException {
        userDAO.updateTravelRadiusKm(userId, Math.max(1, Math.min(50, radiusKm)));
    }
